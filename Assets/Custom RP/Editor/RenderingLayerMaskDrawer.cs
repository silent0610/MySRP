...
public class RenderingLayerMaskDrawer : PropertyDrawer
{
    ...
    )
    {
        ...
        EditorGUI.BeginChangeCheck();//检测GUI变动
        ...
        mask = EditorGUI.MaskField( // 使用 MaskField 方法来绘制一个层遮罩选择控件。允许用户修改
            position, label, mask,
            GraphicsSettings.currentRenderPipeline.renderingLayerMaskNames//调用当前光线定义的mask名称,即在RPasset中定义的
        );
